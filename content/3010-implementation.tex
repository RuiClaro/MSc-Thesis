...
%Figure 1 – Example of a binary DT with depth D=3. Left: usual amount of nodes and leafs. Right: fully expanded tree, with maximum possible number of nodes and leafs
%It is easy to verify that on the tree on the left the paths from the root (the top node) to the different leafs can have different lengths, but on the tree on the right all paths have the same length. This happens because the tree on the left is rather sparse (3 nodes, 4 leafs), while the tree on the right has maximum density (7=2D-1 nodes, 8=2D leafs). Although the use of GC effectively hides the DT evaluation process from unwanted parties, it does not hide the sparseness of the DT, which could leak some relevant information about the original data, meaning that the use of expanded DTs is preferable for preserving privacy. However, this solution implies an exponential increase in total amount of nodes that need to be evaluated, with dramatic consequences in performance.




%3.1.2.	K-MEANS (K-M) + GABLED CIRCUITS (GC)
%Evaluating the k-M algorithm in a privacy-preserving manner is very similar to evaluating k-M in the usual manner. The main difference is the use of ciphered Boolean circuits for replicating all the required operations, namely Euclidean distance and argmin.
